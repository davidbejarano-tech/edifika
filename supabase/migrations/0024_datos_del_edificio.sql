-- =====================================================================
-- EDIFIKA · 0024 Datos del edificio (ajustes del Product Owner, setiembre 2026)
--   1. Ubigeos del Perú (INEI): Departamento › Provincia › Distrito
--   2. Dirección por partes: calle, número, urbanización, referencia, ubigeo y país.
--      La columna "direccion" (recibos, estado de cuenta) se arma sola a partir de las partes.
--   3. Foto y logo del edificio (bucket privado "edificios")
--   4. Datos adicionales: año de construcción y constructora
--   5. Certificaciones con entidad, fechas y archivo
-- Fuente de ubigeos: INEI, vía github.com/jmcastagnetto/ubigeo-peru-aumentado (ubigeo_distrito.csv).
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. Ubigeos (lectura pública: se usan también en el registro)
-- ---------------------------------------------------------------------
create table ubigeos (
  codigo       char(6) primary key check (codigo ~ '^\d{6}$'),
  departamento text not null,
  provincia    text not null,
  distrito     text not null
);
alter table ubigeos enable row level security;
create policy ubigeo_leer on ubigeos for select to anon, authenticated using (true);

-- ---------------------------------------------------------------------
-- 2 y 4. Dirección por partes y datos adicionales
-- ---------------------------------------------------------------------
alter table edificios
  add column calle             text,
  add column numero_calle      text,
  add column urbanizacion      text,
  add column referencia        text,
  add column ubigeo            char(6) references ubigeos,
  add column pais              char(2) not null default 'PE',
  add column anio_construccion smallint,
  add column constructora      text,
  add column foto_path         text,
  add column logo_path         text;
alter table edificios add constraint anio_construccion_valido
  check (anio_construccion is null or anio_construccion between 1800 and 2100);

-- La dirección de una línea (recibos, estado de cuenta, correos) se arma con las partes:
-- "Calle Los Ficus 245, Urb. Corpac, San Isidro, Lima". Los edificios sin partes conservan la suya.
create or replace function tg_direccion_edificio() returns trigger
language plpgsql security definer set search_path = public as $$
declare u ubigeos;
begin
  if coalesce(trim(new.calle), '') = '' then return new; end if;
  select * into u from ubigeos where codigo = new.ubigeo;
  new.direccion := concat_ws(', ',
    trim(concat_ws(' ', trim(new.calle), nullif(trim(new.numero_calle), ''))),
    nullif(trim(new.urbanizacion), ''),
    u.distrito,
    case when u.provincia is not null then u.provincia end);
  return new;
end $$;
create trigger direccion_edificio before insert or update of calle, numero_calle, urbanizacion, ubigeo on edificios
  for each row execute function tg_direccion_edificio();

-- ---------------------------------------------------------------------
-- 5. Certificaciones (ITSE / Defensa Civil, ascensores, pozo a tierra, etc.)
-- ---------------------------------------------------------------------
...
create index certificaciones_edificio on certificaciones (edificio_id, vence_en);
alter table certificaciones enable row level security;
create policy cert_leer on certificaciones for select to authenticated using (es_lector_admin(edificio_id));
create policy cert_titular on certificaciones for all to authenticated
  using (es_titular(edificio_id)) with check (es_titular(edificio_id));

-- ---------------------------------------------------------------------
-- 3. Storage: edificios/{edificio}/foto.*, logo.*, certificaciones/{id}.*
--    Lectura: miembros del edificio (el logo va en los recibos del vecino). Escritura: titular.
-- ---------------------------------------------------------------------
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('edificios', 'edificios', false, 10485760, array['image/jpeg','image/png','image/webp','application/pdf'])
on conflict (id) do nothing;

create policy edificios_leer on storage.objects for select to authenticated using (
  bucket_id = 'edificios' and es_miembro(((storage.foldername(name))[1])::uuid));
create policy edificios_subir on storage.objects for insert to authenticated with check (
  bucket_id = 'edificios' and es_titular(((storage.foldername(name))[1])::uuid));
create policy edificios_actualizar on storage.objects for update to authenticated
  using (bucket_id = 'edificios' and es_titular(((storage.foldername(name))[1])::uuid))
  with check (bucket_id = 'edificios' and es_titular(((storage.foldername(name))[1])::uuid));
create policy edificios_borrar on storage.objects for delete to authenticated using (
  bucket_id = 'edificios' and es_titular(((storage.foldername(name))[1])::uuid));

-- ---------------------------------------------------------------------
-- 1 (datos). 1,893 distritos
-- ---------------------------------------------------------------------
