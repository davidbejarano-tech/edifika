...
export default async function MiEstadoCuentaPage({ searchParams }: { searchParams: Promise<{ p?: string }> }) {
  const { p } = await searchParams;
  const { supabase, actual } = await obtenerContexto();
  if (!actual?.departamento_id) redirect("/edificios");
  return <EstadoCuenta supabase={supabase} actual={actual} p={p} vecino />;
}
