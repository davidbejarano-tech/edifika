...
import { BotonImprimir, Desglose } from "./Controles";
...
// Estado de cuenta del periodo (RN-14, RN-15, RN-24, RN-36). Todos los montos vienen de la base.
export default async function EstadoCuentaPage({ searchParams }: { searchParams: Promise<{ p?: string }> }) {
    ...
    <>
      ...
      <article className="panel relative mx-auto max-w-[820px] print:border-0 print:p-0">
        ...
        <header className="mb-4 border-b border-line pb-3 pr-28">
          ...
          {!oficial && <p className="mt-1 text-xs text-muted">Borrador: los gastos del mes aún no están confirmados y pueden cambiar.</p>}
        </header>
        ...
        <div className="mb-5 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-4">
          ...
          <Cifra titulo="Monto acumulado" valor={soles(r?.acumulado ?? 0)} clase={Number(r?.acumulado ?? 0) >= 0 ? "" : "text-bad"} />
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section>
            ...
            {ingresos?.length ? (
              <Lista filas={ingresos.map((i) => [`${INGRESO[i.tipo] ?? i.tipo} (${i.pagos})`, Number(i.monto)])} total={Number(r?.ingresos ?? 0)} />
            ) : (
        ...
        {custodia > 0 && (
          <p className="mt-4 rounded-lg bg-surface2 px-3 py-2 text-sm">
            <b>Garantías en custodia:</b> {soles(custodia)}. Es dinero de los vecinos por reservas de áreas comunes: no forma parte
        ...
        <Desglose inicial={!!ed?.publicar_desglose}>
          <section className="mt-5">
            ...
            {deudores.length ? (
              <Lista
                filas={deudores.map((c) => [`Depto ${c.numero}${Number(c.vencido) > 0 ? ` · vencido ${soles(c.vencido)}` : ""}`, Number(c.pendiente)])}
                total={deudores.reduce((s, c) => s + Number(c.pendiente), 0)}
              />
            ) : (
              <p className="text-sm text-muted">Todos los departamentos están al día.</p>
            )}
            <p className="mt-1 text-xs text-muted">Al {new Intl.DateTimeFormat("es-PE", { timeZone: "America/Lima", dateStyle: "long" }).format(new Date())}.</p>
          </section>
        </Desglose>
      </article>
    </>
